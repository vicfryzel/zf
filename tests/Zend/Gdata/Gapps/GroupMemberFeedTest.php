...
require_once 'Zend/Gdata/Gapps/GroupMemberFeed.php';
 ...
 */
class Zend_Gdata_Gapps_GroupMemberFeedTest extends PHPUnit_Framework_TestCase
{
    protected $feed = null;
    ...
    public function setUp()
    {
        $feedText = file_get_contents(
                'Zend/Gdata/Gapps/_files/GroupMemberFeedDataSample1.xml',
                true);
        $this->feed = new Zend_Gdata_Gapps_GroupMemberFeed($feedText);
        $this->emptyFeed = new Zend_Gdata_Gapps_GroupMemberFeed();
    }
    ...
    public function testXmlImportAndOutputAreNonDestructive()
    {
        ...
        foreach ($this->feed as $entry) {
            $entryCount++;
            $this->assertTrue(
                    $entry instanceof Zend_Gdata_Gapps_GroupMemberEntry);
        }
        $this->assertTrue($entryCount > 0);

        /* Grab XML from $this->feed and convert back to objects */
        $newFeed = new Zend_Gdata_Gapps_GroupMemberFeed(
                $this->feed->saveXML());
        $newEntryCount = 0;
